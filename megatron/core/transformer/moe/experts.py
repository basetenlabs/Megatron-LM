...
class TEGroupedMLP(MegatronModule):
            ...
            with stash_context:
                ...
                if output_buffer is not None:
                    if OUTPUT_BUFFER_KEY is None:
                        raise RuntimeError("Transformer Engine lacks grouped-linear output buffers")
                    op_kwargs[-1] = {OUTPUT_BUFFER_KEY: output_buffer}
                if grad_input_buffer is not None:
                    if GRAD_INPUT_BUFFER_KEY is None:
                        raise RuntimeError(
                            "Transformer Engine lacks grouped-linear grad-input buffers"
                        )
    ...
    def forward(
        ...
        Args:
            ...
            output_buffer (torch.Tensor, optional): Preallocated symmetric FC2 output buffer.
            grad_input_buffer (torch.Tensor, optional): Preallocated symmetric FC1 dgrad buffer.
        ...
        ), "NCCL-EP zero-copy buffers require the Transformer Engine op-fuser path"
