...
def prime_fp8_indexer_topk(q_indexer, k_indexer, weights, topk, indexer_softmax_scale=1.0):
    """Megatron ``indexer_topk``-compatible wrapper for single-sequence diagnostics."""
    if q_indexer.size(1) != 1:
        raise ValueError(f"Prime FP8 diagnostic indexer supports batch size 1, got {q_indexer.size(1)}.")

    q = q_indexer[:, 0].contiguous()
    k = k_indexer[:, 0].contiguous()
    w = (weights[:, 0].float() * indexer_softmax_scale).to(weights.dtype).contiguous()

    sq = q.shape[0]
    sk = k.shape[0]
    ks = torch.zeros(sq, device=q.device, dtype=torch.int32)
    ke = torch.arange(1, sq + 1, device=q.device, dtype=torch.int32).clamp(max=sk)

    indices = fp8_indexer(q, k, w, ks, ke, topk)
    indices = indices.masked_fill(indices == sk, -1)
    topk_length = (indices >= 0).sum(dim=-1).int()
    return indices.unsqueeze(0).contiguous(), topk_length.unsqueeze(0).contiguous()
