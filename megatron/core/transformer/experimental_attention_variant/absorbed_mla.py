...
class AbsorbedMLASelfAttention(Attention):
    ...
    def _effective_kv_up_weight(self) -> torch.Tensor:
        """Return ``linear_kv_up_proj``'s effective weight, folding in a LoRA adapter.

        The absorbed path consumes the kv up-projection as a raw weight (K's up-projection
        is folded into the query and V's up-projection is applied after core attention)
        instead of calling the module's ``forward``, so a LoRA adapter would otherwise never
        be applied. When the module is LoRA-wrapped (``AdapterWrapper``: a ``to_wrap`` base
        plus an ``adapter``), return ``W_base + scale * (B @ A)`` so the adapter both trains
    ...
    def _get_kv_up_weights(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        weight = self._effective_kv_up_weight()
