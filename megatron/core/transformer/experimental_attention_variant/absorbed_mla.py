...
class AbsorbedMLASelfAttention(Attention):
    ...
    def _kv_up_proj_weight(self) -> torch.Tensor:
        ...
        Override point: the absorbed path uses this weight directly (K folded into the query,
        V applied after core attention) rather than calling ``linear_kv_up_proj.forward``, so
        a LoRA-aware subclass returns an effective weight that folds in the adapter. See
        ``glm_absorbed_mla.GlmAbsorbedMLASelfAttention``.
        """
