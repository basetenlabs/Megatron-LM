...
def unfused_compressed_sparse_attn(
    ...
    * **THD** (3-D query — callers should pre-``squeeze(1)`` the dummy b=1 dim):
    ...
    # Advanced indexing instead of gather-on-expanded-tensor: torch.gather's
    # backward scatter-adds into a zeros_like of the *expanded* (rows, n_kv, hn)
    # operand, allocating/reducing a huge zeros tensor at long seq. kv_flat[idx]
    # scatters into the real (n_kv, hn) kv only. Forward is bitwise-identical.
