...
def indexer_topk(
    ...
    # Frozen-indexer top-k selection (the DSv4 inference/frozen path) only needs the
    # top-k indices, never the dense (b, sq, sk) scores. Use the chunked Triton scorer
    # so the O(seq^2) score tensor (32 GiB at 131k) is never materialized. Falls back
    # to the dense cuDNN path only for the unsupported case (microbatch > 1).
    if q_bshd.shape[0] == 1:
        topk_indices, topk_length = _chunked_indexer_topk_bshd(
...
class FusedIndexerSparseAttnFunc(torch.autograd.Function):
    ...
    * **Dense loss** (``sparse_loss=False``, the default) — KL is
    ...
    def forward(
        ...
        # Trained indexer (loss_coeff > 0): retain the full (b, sq, n_comp) scores
        # for the KL loss + backward. Frozen indexer (loss_coeff == 0, the DSv4
        # recipe default): use the chunked Triton scorer so the O(seq^2) score
        # tensor (32 GiB at 131k) is never materialized — only top-k indices are
        # needed and there is no backward through the indexer.
        if loss_coeff > 0:
        ...
        # indexer_scores is None and there is nothing to compute.
        if loss_coeff > 0:
